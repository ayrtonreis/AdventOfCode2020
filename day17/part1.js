...
function generateSpaceAndBounds(matrix) {
    ...
    for (let x = 0; x < matrix.length; x++) {
        for (let y = 0; y < matrix[0].length; y++) {
            if (matrix[x][y] === '#') {
                space.add(coordToKey([0, x, y]))
            }
        }
    }

    return {space, bounds: [[0, 0], [0, matrix.length - 1], [0, matrix[0].length - 1]]}
}

function getNextPointState(space, [slice, row, col]) {
    ...
    for (let z = minZ; z <= maxZ; z++) {
        for (let x = minX; x <= maxX; x++) {
            for (let y = minY; y <= maxY; y++) {
                if (space.has(coordToKey([z, x, y])) && !(z === slice && x === row && y === col)) {
    ...
    if (space.has(coordToKey([slice, row, col]))) return countActiveNeighbors === 2 || countActiveNeighbors === 3

    return countActiveNeighbors === 3
}

function stepSpace(prevSpace, [[minZ, maxZ], [minX, maxX], [minY, maxY]]) {
    ...
    for (let z = minZ - 1; z <= maxZ + 1; z++) {
        for (let x = minX - 1; x <= maxX + 1; x++) {
            for (let y = minY; y <= maxY + 1; y++) {
                const isActive = getNextPointState(prevSpace, [z, x, y])

                if (isActive) {
                    nextSpace.add(coordToKey([z, x, y]))
                    ...
                    nextSpace.delete(coordToKey([z, x, y]))
                }
            }
        }
    }

    return {space: nextSpace, bounds: [[minZ - 1, maxZ + 1], [minX - 1, maxX + 1], [minY - 1, maxY + 1]]}
